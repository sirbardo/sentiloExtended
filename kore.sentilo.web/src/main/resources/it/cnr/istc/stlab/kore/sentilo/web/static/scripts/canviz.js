...
//					debug('processing token ' + token);
					switch (token) {
						...
						case 'T': // text
							/*
							var ltmp = tokenizer.takeNumber();
							var btmp = this.canviz.height - tokenizer.takeNumber();
							var wtmp = tokenizer.takeNumber();
							var htmp = tokenizer.takeNumber();
							var fontSizeTmp = Math.round(fontSize * ctxScale) + "px";
							var fontFamilyTmp = fontFamily;
							var colorTmp = strokeColor.textColor;
							
							var contextTmp = this.canviz.canvas.getContext("2d");
							contextTmp.fillStyle = colorTmp;
							contextTmp.font = fontSizeTmp + " " + fontFamilyTmp;
							
							var strTmp = tokenizer.takeString();
							contextTmp.fillText(strTmp.escapeHTML(), ltmp, btmp);
							*/
							
							var ltmp = tokenizer.takeNumber();
							var btmp = tokenizer.takeNumber();
							var fontSizeTmp = fontSize + "px";
							
							//var l = Math.round(ctxScale * tokenizer.takeNumber() + this.canviz.padding);
							var l = Math.round(ctxScale * ltmp + this.canviz.padding);
							var t = Math.round(ctxScale * this.canviz.height + 2 * this.canviz.padding - (ctxScale * (btmp + this.canviz.bbScale * fontSize) + this.canviz.padding));
							
							var btmp = this.canviz.height - btmp;
...
//									debug(this.name + ', href ' + href + ', target ' + target + ', tooltip ' + tooltip);
								...
								text.update(str);
								text.setStyle({
									fontSize: (Math.round(fontSize * ctxScale * this.canviz.bbScale)-1) + 'px',
							...
							//var fontSizeTmp = Math.round(fontSize * ctxScale * this.canviz.bbScale) + "px";
							//var fontSizeTmp = Math.round(fontSize * ctxScale * this.canviz.bbScale) + "px";
							/*
							var fontFamilyTmp = fontFamily;
							var colorTmp = strokeColor.textColor;
							...
							contextTmp.fillStyle = colorTmp;
							contextTmp.font = fontSizeTmp + " " + fontFamilyTmp;
							contextTmp.fillText(str.escapeHTML(), (ltmp - (1 + textAlign) * textWidth), btmp);
							*/
							
							var label = {
								content: str.escapeHTML(),
								left: (ltmp - (1 + textAlign) * textWidth),
								top: btmp,
								font: fontSizeTmp + " " + fontFamily
							};
							
							this.canviz.labels.push(label);
							...
							contextTmp.fillText(str.escapeHTML(), ltmp, btmp);
							*/
							break;
...
var CanvizImage = Class.create({
	...
	onFinish: function() {
		...
		if (this.canviz.numImages == this.canviz.numImagesFinished) {
			...
			if(this.canviz.drawLabels){
				var ctx = this.canviz.canvas.getContext("2d");
				var spans = jQuery("#exportCanvas div span");
				for(var i=0; i<spans.length; i++){
				
					var fontSize = jQuery(spans[i]).css("font-size");
					if(i==0){
						var fontFamily = jQuery(spans[i]).css("font-family");
						ctx.font = fontSize + " " + fontFamily;
					}
					
					var align = jQuery(spans[i]).css("text-align");
					
					var textAlign= ("left" == align) ? -1 : ("right" == align) ? 1 : 0;
					
					var content = jQuery(spans[i]).html();
					
					jQuery(spans[i]).html("<span>"+content+"</span>");
					
					var s = jQuery(spans[i]).find('span:first');
					var textWidth = jQuery(s).width()/2;
					
					jQuery(spans[i]).html(content);
					
					var left = parseInt(jQuery(spans[i]).css("left").replace("px", "")) + ((1 + textAlign) * textWidth);
					var top = parseInt(jQuery(spans[i]).css("top").replace("px", "")) + 16;
					
					ctx.fillText(content, left, top);
					ctx.save();
				}
				
				var spans = jQuery("#exportCanvas div a");
				for(var i=0; i<spans.length; i++){
				...
				jQuery("#exportCanvas").hide();
				
				if(this.canviz.mimetype == "image/svg+xml"){
					
				}
				else{
					window.location = this.canviz.canvas.toDataURL(this.canviz.mimetype);	
				}
